...
<button class="tab-btn active" onclick="setPanelTab(event, 'geral')">Dados</button>
<button class="tab-btn" onclick="setPanelTab(event, 'estilo')">Estilo</button>
<button class="tab-btn" onclick="setPanelTab(event, 'camadas')">Camadas</button>
</div>
...
<div class="form-group"><label class="form-label">Eixo Y (Métrica)</label><select id="sel-y" class="form-select" disabled><option value="(Contagem)">(Contagem)</option></select></div>
<div class="form-group"><label class="form-label">Tipo</label><select id="sel-type" class="form-select"><option value="bar">Barras</option><option value="pie">Pizza</option><option value="scatter">Dispersão</option><option value="histogram">Histograma</option></select></div>
<div class="chk-wrapper" title="Se marcado, calcula a média dos registos de cada paciente antes de agregar."><input type="checkbox" id="chk-agrupar-paciente"><span>Agrupar por Paciente</span></div>
...
<span class="material-icons-round">play_arrow</span> ATUALIZAR
</button>

<div id="adjust-template-container" style="display: none; margin-top: 10px;">
<button class="btn-stack" onclick="openAdjustTemplateScreen()" style="width:100%; background:#fffbe6; color:#b45309; border-color:#fde68a;">
    <span class="material-icons-round">edit</span> Ajustar Modelo
</button>
</div>

<div style="display:flex; gap:8px; margin-top: 8px;">
<button class="btn-stack" onclick="sendChartToReport()" style="width:100%;">
  📌 Add Relatório
</button>
<button class="btn-magic" onclick="openAnalysisModal()" style="margin:0; width:100%;">
    <span class="material-icons-round">auto_awesome</span> Assistente
</button>
</div>
...
<div class="form-group"><label class="form-label">Título</label><input type="text" id="style-title" class="form-input" placeholder="Automático"></div>
<div class="form-group"><label class="form-label">Cor</label><input type="color" id="style-color" class="color-input" value="#3b82f6"></div>
<div class="form-group"><label class="form-label">Rótulo X</label><input type="text" id="style-xaxis" class="form-input" placeholder="Automático"></div>
<div class="form-group"><label class="form-label">Rótulo Y</label><input type="text" id="style-yaxis" class="form-input" placeholder="Automático"></div>
<div class="form-group" style="border-top: 1px solid #f1f5f9; padding-top: 15px; margin-top:15px;">
  <label class="form-label">Sobreposições Estatísticas</label>
  <div class="chk-wrapper"><input type="checkbox" id="style-show-mean"><span>Exibir Linha da Média</span></div>
  <div class="chk-wrapper"><input type="checkbox" id="style-show-median"><span>Exibir Linha da Mediana</span></div>
</div>
...
<div style="display:flex; gap:10px; align-items:center;">
  ...
  <button onclick="exportGlobalExcel()" style="background:#fff; border:1px solid #cbd5e1; color:#334155; padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:600; display:flex; align-items:center; gap:6px; font-size:0.8rem; box-shadow:0 1px 2px rgba(0,0,0,0.05); white-space:nowrap;">
    <span class="material-icons-round" style="font-size:16px; color:#10b981;">table_view</span> 
    Excel Geral
  </button>

  <!-- ✅ CORRIGIDO: chama openDoseModal() do novo LabRegrasDose -->
  <button id="btn-dose-audit" onclick="openDoseModal()" style="background:#fff; border:1px solid #cbd5e1; color:#334155; padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:600; display:flex; align-items:center; gap:6px; font-size:0.8rem; box-shadow:0 1px 2px rgba(0,0,0,0.05); white-space:nowrap;">
      <span class="material-icons-round" style="font-size:16px; color:#ef4444;">error_outline</span>
      Auditoria de Dose
  </button>
...
<div class="view-container">
  ...
  <div id="view-grid" class="dashboard-grid">
      <div class="dash-card">
        <div class="dash-title">DEMOGRAFIA (PACIENTES)</div>
        <div id="chart-1" style="flex:1;"></div>
      </div>
      <div class="dash-card">
        <div class="dash-title">TOP MEDICAMENTOS</div>
        <div id="chart-2" style="flex:1;"></div>
      </div>
      <div class="dash-card">
        <div class="dash-title">EVOLUÇÃO TEMPORAL</div>
        <div id="chart-3" style="flex:1;"></div>
      </div>
      <div class="dash-card">
        <div class="dash-title">DISTRIBUIÇÃO IDADE (PACIENTES)</div>
        <div id="chart-4" style="flex:1;"></div>
      </div>
  </div>
</div>
...
<div class="oracle-header"><span>Oráculo</span><span class="material-icons-round" style="cursor:pointer;" onclick="toggleOracle()">close</span></div>
<div class="oracle-body" id="chat-body"><div class="chat-bot chat-bubble">Olá! Posso ajudar a filtrar os dados.</div></div>
...
<div>
<h3 class="drill-title">Detalhes do Grupo</h3>
<div id="drill-subtitle" class="drill-subtitle"></div>
</div>
...
<button onclick="exportDrillExcel()" style="padding:8px 16px; border-radius:6px; border:1px solid #10b981; background:#ecfdf5; color:#047857; cursor:pointer; font-weight:600;">📥 Excel</button>
<button onclick="sendTableToReport()" style="padding:8px 16px; border-radius:6px; border:1px solid #3b82f6; background:#eff6ff; color:#1d4ed8; cursor:pointer; font-weight:600;">📌 Enviar p/ Relatório</button>
</div>
...
<?!= include('LabAnaliseRapidaModal'); ?>
<?!= include('LabDoseAuditModal'); ?>
...
let lastUsedTemplateId = null; 
window.latestChartConfig = {}; 

// ================================================================
// REMOVIDO: openDoseAuditConfig() e closeDoseAuditConfig()
// O botão "Auditoria de Dose" agora chama openDoseModal()
// definida em LabRegrasDose (incluído acima).
// ================================================================

function setLastUsedTemplate(templateId) {
lastUsedTemplateId = templateId;
const container = document.getElementById('adjust-template-container');
if (container) {
    container.style.display = 'block';
}
}

function openAdjustTemplateScreen() {
if (lastUsedTemplateId && typeof showTemplateConfig === 'function') {
    showTemplateConfig(lastUsedTemplateId);
}
}
...
if(res.config.core && res.config.core.abaFatos) {
  ...
  loadCols();
} else { sel.innerHTML = '<option>⚠️ Configure no Wizard</option>'; }
...
opts += `<optgroup label="⚙️ Auto"><option value="_IDADE_ANOS">Idade (Anos)</option><option value="_PID">ID Paciente</option></optgroup>`;
document.getElementById('sel-x').innerHTML='<option value="">-- Selecione --</option>'+opts; document.getElementById('sel-x').disabled=false;
document.getElementById('sel-y').innerHTML='<option value="(Contagem)">(Contagem)</option>'+opts; document.getElementById('sel-y').disabled=false;
}).apiGetColumns(sheet);
}
...
if(found===-1){ sel.add(new Option(`✨ ${val}`,val)); sel.value=val; } else { sel.selectedIndex=found; }
return true;
}
...
const showMean = document.getElementById('style-show-mean').checked;
const showMedian = document.getElementById('style-show-median').checked;

const useYCol = yCol && yCol !== '(Contagem)';
currentXAxis = xCol;
...
if(useYCol) colsNeed.push(yCol);

const virtualFieldNames = (activeVirtualFields || []).map(vf => vf.name);
colsNeed = [...new Set(colsNeed)].filter(c => c && !virtualFieldNames.includes(c));

google.script.run
.withFailureHandler(err => {
   btn.innerHTML = '⚠️ Erro Fatal'; btn.disabled = false;
   alert("Ocorreu um erro de conexão ou timeout:\n" + err.message);
})
.withSuccessHandler(res => {
  if(!res.sucesso) {
     btn.innerHTML = '⚠️ Erro Dados'; btn.disabled = false;
  ...
  if (activeVirtualFields && activeVirtualFields.length > 0) {
      data = data.map(row => {
          const newRow = Object.assign({}, row);
          activeVirtualFields.forEach(vf => {
              try {
                  const fn = new Function('row', vf.formula);
                  newRow[vf.name] = fn(newRow);
              } catch(e) {
                  newRow[vf.name] = '(ERRO na fórmula)';
              }
          });
          return newRow;
      });
  ...
  if(isGrouped) {
    ...
    data.forEach(r => {
      ...
      if(useYCol) groups[pid].y.push(r[yCol]);
    ...
    data = Object.keys(groups).map(pid => {
      const g = groups[pid];
      const vx = g.x[0]; 
      let vy = useYCol ? (g.y.reduce((a,b)=>a+(parseFloat(b)||0),0)/g.y.length) : g.x.length;
      const obj={}; obj[xCol]=vx; obj[useYCol ? yCol : 'COUNT']=vy; obj['_PID']=pid; obj._rawRows = g.raw; 
      return obj;
    });
  }
  ...
  data.forEach(d => {
    let cat = d[xCol];
    if (cat === undefined || cat === null) cat = 'N/D';
    
    if (!categories[cat]) categories[cat] = { sum: 0, count: 0 };
    const val = useYCol ? (parseFloat(String(d[yCol]).replace(',','.'))||0) : 1;
    categories[cat].sum += val; 
    categories[cat].count++;
  });
  
  finalX = Object.keys(categories).sort();
  finalY = finalX.map(c => useYCol ? (categories[c].sum / categories[c].count) : categories[c].count);

  const patientMap = new Map();
  res.dados.forEach(row => {
      if (!patientMap.has(row['_PID'])) {
          patientMap.set(row['_PID'], row['_SEXO']);
      }
  });
  let malePatients = 0;
  let femalePatients = 0;
  for (const sexo of patientMap.values()) {
      if (sexo === 'M') malePatients++;
      else if (sexo === 'F') femalePatients++;
  }
  document.getElementById('kpi-n').innerText = res.dados.length;
  document.getElementById('kpi-p').innerText = patientMap.size;
  document.getElementById('kpi-male').innerText = malePatients || '-';
  document.getElementById('kpi-female').innerText = femalePatients || '-';

  const trace = {
    x: finalX, y: finalY,
    type: type==='scatter'?'scatter':(type==='pie'?'pie':'bar'),
    mode: type==='scatter' ? 'markers' : undefined,
    name: useYCol ? 'Média' : 'Contagem',
  ...
  const layout = { 
    title: customTitle || `${useYCol ? yCol : 'Contagem'} por ${xCol}` + (isGrouped ? " (Por Paciente)" : ""),
    xaxis: { title: customX || xCol, automargin: true },
    yaxis: { title: customY || (useYCol ? `Média de ${yCol}` : 'Quantidade') },
    hovermode: 'closest',
    margin: {t:40, b:80, l:50, r:80},
    autosize: true,
    shapes: [],
    annotations: []
  };
  
  if ((showMean || showMedian) && type !== 'pie') {
      let valuesForStats = [];
      if (useYCol) {
          valuesForStats = data.map(d => parseFloat(d[yCol])).filter(n => !isNaN(n));
      } else {
          valuesForStats = Object.values(categories).map(c => c.count);
      }
      
      if(valuesForStats.length > 0) {
          if (showMean) {
              const mean = valuesForStats.reduce((a, b) => a + b, 0) / valuesForStats.length;
              layout.shapes.push({
              ...
              layout.annotations.push({
                  xref: 'paper', yref: 'y',
                  x: 1.01, y: mean, text: `Média: ${mean.toFixed(1)}`,
          ...
          if (showMedian) {
              ...
              layout.shapes.push({
                  type: 'line', xref: 'paper',
                  x0: 0, y0: median, x1: 1, y1: median,
                  line: { color: '#22c55e', width: 2, dash: 'dot' }
              });
              layout.annotations.push({
                  xref: 'paper', yref: 'y',
                  x: 1.01, y: median, text: `Mediana: ${median.toFixed(1)}`,
                  showarrow: false, xanchor: 'left',
                  font: {color: '#22c55e', size: 10},
                  yshift: showMean ? -15 : 0 
              });
          }
      }
  }
  ...
  window.latestChartConfig = { data: [trace], layout: layout }; 
...
}).apiGetRawExplorerData(sheet, colsNeed, activeFilterScript);
}

function runDoseAudit(config) {
    openDoseAudit([]);
    const modalTitle = document.getElementById('dose-audit-title');
    if(modalTitle) modalTitle.innerText = "Executando auditoria...";

    google.script.run
        .withSuccessHandler(problemas => {
            if(modalTitle) modalTitle.innerText = "Auditoria de Doses";
            openDoseAudit(problemas);
        })
        .withFailureHandler(err => {
            if(modalTitle) modalTitle.innerText = "Erro na Auditoria";
            alert("Ocorreu um erro ao executar a auditoria: " + err.message);
            closeDoseAudit();
        })
        .runDoseAudit(config);
}

function runAnalysisWithCustomRanges(ranges) {
if (!ranges || ranges.length === 0) {
    return alert("Por favor, defina ao menos uma faixa etária.");
}

ranges.sort((a, b) => parseFloat(String(a.min).replace(',', '.')) - parseFloat(String(b.min).replace(',', '.')));

var formulaBody = ranges
    .map(r => {
        var min = parseFloat(String(r.min).replace(',', '.'));
        var max = parseFloat(String(r.max).replace(',', '.'));
        
        if (isNaN(min) || isNaN(max)) {
            return null;
        }
        
        var label = String(r.label || 'Sem Rótulo').replace(/'/g, "\\'");
        return `if (age >= ${min} && age <= ${max}) return '${label}';`;
    })
    .filter(Boolean)
    .join(' else ');

if (!formulaBody) {
    return alert("Nenhuma das faixas definidas é válida. Verifique os valores de idade mínima e máxima.");
}

const fullFormula = `
try {
var rawValue = row['_IDADE_ANOS'];

if (rawValue === undefined || rawValue === null || String(rawValue).trim() === '') {
return '(Idade não informada)';
}

var age = parseFloat(String(rawValue).trim().replace(',', '.'));

if (isNaN(age)) {
return '(Idade com valor inválido)';
}

${formulaBody}
else {
return '(Fora das faixas definidas)';
}
} catch (e) {
return '(ERRO na execução da fórmula)';
}
`;

const virtualField = {
    name: '_FAIXA_ETARIA_CUSTOM',
    formula: fullFormula
};

activeVirtualFields = [virtualField];
activeFilterScript = null; 

injectAndSelect('sel-x', '_FAIXA_ETARIA_CUSTOM');
injectAndSelect('sel-y', '(Contagem)');
injectAndSelect('sel-type', 'bar');

document.getElementById('chk-agrupar-paciente').checked = false;
document.getElementById('active-filter-badge').style.display = 'none';
...
btn.innerHTML = '⏳ IA ANALISANDO...'; btn.disabled = true;

google.script.run.withSuccessHandler(state => {
 const config = state.config;
 const smartPlan = config.smartCharts || []; 
 const mapping = config.core || {};
 
 const chartsToRender = (smartPlan.length === 4) ? smartPlan : [
    { title: "DEMOGRAFIA (SEXO)", type: "pie", x: "_SEXO" },
    { title: "TOP MEDICAMENTOS", type: "bar", x: "colMed" },
    { title: "EVOLUÇÃO TEMPORAL", type: "line", x: "colDtIni" },
    { title: "DISTRIBUIÇÃO ETÁRIA", type: "histogram", x: "_IDADE_ANOS" }
 ];

 chartsToRender.forEach(c => {
    c.realX = mapping[c.x] || c.x; 
    c.realY = mapping[c.y] || c.y;
    if (['_IDADE_ANOS', '_SEXO', '_PID'].includes(c.x)) c.realX = c.x;
 });
 const realCols = chartsToRender.flatMap(c => [c.realX, c.realY]).filter(Boolean);
...
let trace = {};
let layout = { margin: {t:20, b:30, l:40, r:10}, autosize: true, showlegend: false };
...
currentXCol = xCol;
modal.showModal(); overlay.style.display = 'block';
...
currentAuditInfo = {
 "Pergunta Original": document.getElementById('filter-desc')?.innerText || "Nenhum",
 "População": `${uniqueCount} Pacientes (de ${rawCount} registos)`,
 "Fórmula (Excel)": excelFriendly,
 "Idade Média": `${avgAge} anos`,
 "Distribuição Sexo": `M: ${sexDist.M} | F: ${sexDist.F}`,
 "Data Geração": new Date().toLocaleString()
};
...
auditDiv.innerHTML = `
<div class="audit-item"><span class="audit-label">Estatísticas</span>
  <div>Média Idade: <span class="audit-val">${avgAge}</span></div>
...
<div class="audit-item"><span class="audit-label">Lógica Aplicada</span>
...
const useYColDrill = yCol && yCol !== '(Contagem)';

let headHtml = `<tr><th>ID</th><th>Sexo</th><th>Data</th><th>${xCol}</th>`;
if(useYColDrill) headHtml += `<th>${yCol}</th>`;
...
rowsToShow.slice(0, 100).forEach(r => {
 ...
 let sexHtml = sex === 'M' ? `<span style="color:#2563eb;background:#eff6ff;padding:2px 6px;border-radius:4px;font-weight:bold">M</span>` : (sex === 'F' ? `<span style="color:#db2777;background:#fce7f3;padding:2px 6px;border-radius:4px;font-weight:bold">F</span>` : sex);
 let valX = `<span class="val-highlight">${r[xCol]}</span>`;
 let valY = useYColDrill ? (typeof r[yCol]==='number' ? r[yCol].toFixed(2) : (r[yCol] || '-')) : '-';
 let ctx = '';
 if(r['MEDICAMENTO']) ctx += `<span class="ctx-badge">${r['MEDICAMENTO']}</span>`;

 let row = `<tr><td>${pid}</td><td>${sexHtml}</td><td>${dt}</td><td>${valX}</td>`;
 if(useYColDrill) row += `<td>${valY}</td>`;
 row += `<td>${ctx}</td></tr>`;
 tbody.innerHTML += row;
});
}
...
if (!currentDrillData || currentDrillData.length === 0) return alert("Sem dados.");
const wb = XLSX.utils.book_new();
const auditRows = Object.keys(currentAuditInfo).map(key => ({ "Parâmetro": key, "Valor": currentAuditInfo[key] }));
...
XLSX.utils.book_append_sheet(wb, wsSelected, "2. Seleção_Atual");
XLSX.writeFile(wb, "EUM_Dossie.xlsx");
}
...
function sendChartToReport() {
  ...
  if (!div || !div.data) return alert("Sem gráfico para adicionar. Gere um gráfico primeiro.");
 
  const titleEl = div.querySelector('.gtitle');
  const title = titleEl ? titleEl.textContent : (document.getElementById('style-title').value || 'Gráfico');
  ...
  if (typeof window.addToReport === 'function') {
    ...
    alert("Erro: módulo de Relatório não encontrado. Verifique se LabRelatorio está incluído no LabHome.");
  }
}

function sendTableToReport() {
  if (!currentDrillData || currentDrillData.length === 0) {
    return alert("Sem dados na tabela para enviar.");
  }
  ...
  const cleanData = currentDrillData.map(r => {
    ...
    row["Sexo"] = r['_SEXO'] || '-';
    row["Data"] = safeDate(r['DT_REFERENCIA']);
    if (currentXCol && !['SEXO', 'DATA'].includes(currentXCol)) {
...
.withFailureHandler(err => {
  ...
  addChatBubble(`⚠️ **Erro na Execução**<br>${err.message}`, 'bot');
})
.withSuccessHandler(res => {
  document.getElementById(loadId).remove();
  if(res.sucesso) {
    if (res.config) {
      ...
      activeFilterScript = cfg.filter; 
      activeVirtualFields = cfg.virtualFields || [];
      
      injectAndSelect('sel-x', cfg.xAxis); 
      injectAndSelect('sel-y', cfg.yAxis || ""); 
      injectAndSelect('sel-type', cfg.chartType);
      
      if (cfg.group !== undefined) document.getElementById('chk-agrupar-paciente').checked = !!cfg.group;
      
      addChatBubble(`📊 ${res.explanation || 'Ok, entendi. Gerando o gráfico...'}`, 'bot');
      ...
      addChatBubble(res.response, 'bot');
    } else {
       addChatBubble("Não entendi o que fazer.", 'bot');
    }
  } else {
    addChatBubble(`⚠️ **Erro do Oráculo**<br>${res.erro}`, 'bot');
  }
...
let human = jsCode.replace(/row\['(.*?)'\]/g, "$1");
...
human = human.replace(/_IDADE_ANOS/g, "IDADE").replace(/_SEXO/g, "SEXO").replace(/_PID|PRONTUARIO/g, "ID_PACIENTE").replace(/DT_REFERENCIA/g, "DATA_REF");
human = human.replace(/&&/g, " E ").replace(/\|\|/g, " OU ").replace(/==/g, " _IGUAL_ ").replace(/!=/g, " _DIFERENTE_ ").replace(/>=/g, " _MAIOR_IGUAL_ ").replace(/<=/g, " _MENOR_IGUAL_ ").replace(/>/g, " _MAIOR_ ").replace(/</g, " _MENOR_ ");
return human;
}
...
if (typeof XLSX === 'undefined') return alert("Erro: Biblioteca Excel não carregada.");
if (!globalChartData || globalChartData.length === 0) return alert("Ainda não há dados carregados.");
const sheet = document.getElementById('sel-source').value;
...
if(btn) { 
 btn.innerHTML = '<span class="material-icons-round" style="font-size:16px; animation:spin 1s linear infinite;">sync</span> Baixando...'; 
 btn.disabled = true; 
}
google.script.run
.withSuccessHandler(res => {
...
.withFailureHandler(err => {
   if(btn) { btn.innerHTML = originalText; btn.disabled = false; }
   alert("Erro fatal de conexão: " + err.message);
})
.apiGetRawExplorerData(sheet, colsToFetch, null, []); 
}

function generateTrueAuditExcel(fullData) {
try {
 const wb = XLSX.utils.book_new();
 let prompt = document.getElementById('filter-desc')?.innerText || "Nenhum";
 let formula = (activeFilterScript && activeFilterScript !== 'true') ? activeFilterScript : "Nenhuma (Todos os dados)";
 let excelFriendly = translateJsToHuman(formula);

 const auditRows = [
     { "Parâmetro": "Pergunta", "Valor": prompt },
     { "Parâmetro": "População Filtrada", "Valor": globalChartData.length },
     { "Parâmetro": "População Total", "Valor": fullData.length },
     { "Parâmetro": "Fórmula Auditoria", "Valor": excelFriendly },
 ...
 const wsData = XLSX.utils.json_to_sheet(fullData.map(r => formatDataForExcel([r])[0]));
 XLSX.utils.book_append_sheet(wb, wsData, "2. Dados_Brutos_Completo");
